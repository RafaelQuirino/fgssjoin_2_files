...
)
{
    ...
    for (int i = idx; i < n; i += blockDim.x * gridDim.x)
    {
        ...
        unsigned int query_jac_prefix_size =
            jac_max_prefix (query_len, threshold);
		unsigned int query_jac_min_size =
            (unsigned int) floor(jac_min_size(query_len, threshold));
        unsigned int query_jac_max_size =
            (unsigned int) floor(jac_max_size(query_len, threshold));
		...
		for (unsigned int j = 0; j < query_jac_prefix_size; j++)
		{
			...
			for (unsigned int k = 0; k < list_size; k++)
			{
				...
				if (query < source)
                {
                    ...
                    unsigned int source_jac_min_size =
                        (unsigned int) floor(jac_min_size(source_len, threshold));
                    unsigned int source_jac_max_size =
                        (unsigned int) floor(jac_max_size(source_len, threshold));
                    ...
                    if (score >= 0)
                    {
                        ...
                        float jac_minoverlap =
                            jac_min_overlap (query_len, source_len, threshold);
