...
)
{
    ...
    for (unsigned i = 0; i < m; i++) {
        ...
        if (order == ORDER_1)
            lists[i].pos = inv_index->lists[i]->pos_1;
        else
            lists[i].pos = inv_index->lists[i]->pos_2;
    }
